  ...
  export default {
    ...
    data() {
      ...
      return {
        tableData: [],
        roleMap: {
          0: { type: 'danger', text: '超级管理员' },
          1: { type: 'primary', text: '管理员' },
          2: { type: 'success', text: '普通用户' },
          3: { type: 'warning', text: '访客' }
          // 可以继续扩展更多值
        },
        pageSize:17,
        pageNum:1,
    ...
    methods:{
      ...
      headerStyle(){
        // console.log(Index)
        return 'background:rgb(245, 246, 247);text-align:center;'
      }
    },
    beforeMount() {
      // this.loadGet();
...
<template>
    <div>
      <div style="margin-bottom: 5px;">
        ...
        <el-select style="margin-left: 5px;" v-model="sex" placeholder="请选择性别">
          <el-option
              v-for="item in sexs"
              :key="item.value"
              :label="item.label"
      ...
      >
        ...
        <el-table-column prop="no" label="账号" min-width="180">
        </el-table-column>
        ...
        <el-table-column prop="roleId" label="角色" min-width="180">
          <template slot-scope="scope">
            <el-tag
                :type="roleMap[scope.row.roleId]?.type"
                disable-transitions>{{ roleMap[scope.row.roleId]?.text}}</el-tag>
          </template>
        </el-table-column>
